...
void MapLayerImage::update( EventSystem& events, BlockSystem& blocks, const Camera& camera, Map& lvmap )
{
	...
	if ( movement_position_x_ < -Unit::PixelsToSubPixels( lvmap.widthPixels() ) - Unit::PixelsToSubPixels( offset_x_ ) )
	{
		/*
		...
		*/
		...
		const int times_to_push_forward = ceil( ( Unit::PixelsToSubPixels( lvmap.widthPixels() ) - movement_position_x_ ) / amount_to_push_forward );
...
void MapLayerImage::renderY( const Camera& camera ) const
{
	int dy = offset_y_ - ( (int)( camera.y() * scroll_speed_y_ ) ) + Unit::SubPixelsToPixels( movement_position_y_ );
	int dl = offset_x_ - ( (int)( camera.x() * scroll_speed_x_ ) ) + Unit::SubPixelsToPixels( movement_position_x_ );// + ( source_.w * current_frame_.value() ); // Quite certain commented out part has no reason to be here.
	...
	if ( repeat_y_ > 0 )
	{
		...
		int i = 0;
		while ( dest.y < camera.heightPixels() && ( repeat_y_ == REPEAT_INFINITE || i < repeat_y_ ) )
		{
			dest.x = dl;
			dest.y += source_.h;
			renderX( camera, dest );
...
void MapLayerImage::renderX( const Camera& camera, sdl2::SDLRect& dest ) const
{
	...
	{
		int i = 0;
		while ( dest.x < camera.widthPixels() && ( repeat_x_ == REPEAT_INFINITE || i < repeat_x_ ) )
		{
			dest.x += source_.w;
