...
//===================================
    ...
    :
        Sprite( std::unique_ptr<SpriteGraphics> ( new HydrantGraphics() ), x, y, 14, 14, SpriteType::ENEMY, 100, 1500, 3000, 3000 )
    {
        jump_lock_ = false;
    };
    ...
    void HydrantSprite::customUpdate( Input& input, Camera& camera, Map& lvmap, Game& game, EventSystem& events, SpriteSystem& sprites )
    {
        if ( isDead() )
